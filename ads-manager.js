...
class AdsManager {
  ...
  async loadAllAds() {
    ...
    // 1. Popunder & Click-triggered Smartlink (Immediate: zero delay so first click/touch is captured!)
    this.loadPopunder();
    this.loadSmartlink();
    
    // 2. Sidebar Native banner & Banners (Immediate)
  ...
  loadNativeBanner() {
    ...
    if (sidebar) {
      ...
      if (grid && !document.getElementById('ad-native-grid')) {
        ...
        container.style.cssText = 'padding: 15px; display: flex; flex-direction: column; justify-content: center; align-items: center; background: rgba(22, 28, 45, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: var(--card-radius, 14px); min-height: 250px; position: relative; overflow: hidden;';
        ...
        const gameCards = Array.from(grid.children).filter(c => c.classList.contains('card') && !c.classList.contains('ad-banner'));
        if (gameCards.length >= 9) {
          grid.insertBefore(container, gameCards[8]);
          // Remove the last game card on the page so total items remain balanced and evenly fill all columns (e.g. exactly 24 items)
  ...
  loadSocialBar() {
    ...
    }, this.config.socialBar.delay || 1500);
  }
  ...
  loadSmartlink() {
    if (!this.config.smartlink?.enabled || !this.config.smartlink?.url) return;
    this.setupSmartlinkPopunder();
  }

  setupSmartlinkPopunder() {
    const smartUrl = this.config.smartlink.url;
    if (!smartUrl) return;
    
    const triggerSmartlink = (e) => {
      try {
        const win = window.open(smartUrl, '_blank');
        if (win) {
          win.blur();
          window.focus();
        }
      } catch (err) {
        console.warn('Smartlink trigger error:', err.message);
      }
      window.removeEventListener('click', triggerSmartlink, true);
      window.removeEventListener('touchend', triggerSmartlink, true);
    };
    
    window.addEventListener('click', triggerSmartlink, { capture: true, once: true });
    window.addEventListener('touchend', triggerSmartlink, { capture: true, once: true });
  }
